    ...
    <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', flexDirection: 'column' }}>
      ...
      {showPublicModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: T.card, border: `1px solid ${T.borderMid}`, borderRadius: 20, padding: 24, maxWidth: 360, width: '100%' }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: T.text, fontFamily: F.sans, marginBottom: 8 }}>Make Course Public?</div>
            <div style={{ fontSize: 13, color: T.textSub, fontFamily: F.sans, lineHeight: 1.7, marginBottom: 16 }}>Your course will appear in the Explore tab for all users to enrol in.</div>
            <div style={{ background: T.card2, border: `1px solid ${T.border}`, borderRadius: 10, padding: 14, marginBottom: 16 }}>
              <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: 12, color: T.textSub, fontFamily: F.sans, lineHeight: 1.6 }}>I confirm this content is my own or I have the right to share it. I understand sharing copyrighted material without permission may result in removal. <a href="/terms" target="_blank" style={{ color: T.teal }}>Terms of Service</a></span>
              </label>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { setShowPublicModal(false); setPublicConfirmed(false); }} style={{ flex: 1, padding: 11, background: T.card2, border: `1px solid ${T.border}`, borderRadius: 10, color: T.textSub, fontFamily: F.sans, fontSize: 13, cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => { setShowPublicModal(false); togglePublic(); setPublicConfirmed(false); }} disabled={!publicConfirmed} style={{ flex: 1, padding: 11, background: publicConfirmed ? T.teal : T.card2, border: 'none', borderRadius: 10, color: publicConfirmed ? T.btnText : T.muted, fontFamily: F.sans, fontSize: 13, fontWeight: 600, cursor: publicConfirmed ? 'pointer' : 'not-allowed' }}>Make Public</button>
            </div>
          </div>
        </div>
      )}
