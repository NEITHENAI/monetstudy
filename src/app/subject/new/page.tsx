...
import { getTopics, updateTopic, updateSubject, createSubject, saveTopic, generateSubjectId } from '@/lib/firebase/firestore';
...
export default function NewSubjectPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Bypass Next.js POST limits for massive books
      if (text.length > 50000) {
        setStage('Uploading massive textbook...');
        materialUrl = await uploadSourceMaterial(user.uid, subjectId, text);
        payloadText = ''; // Dont send the massive string in JSON body
      }

      setStage('Building course from material...');
        ...
        {step === 'personalise' && (
          <div style={{ animation: 'fadeUp 0.3s ease' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <MonoLabel size={10} color={T.violet}>Custom Instructions (Optional)</MonoLabel>
              <p style={{ color: T.textSub, fontSize: 12, margin: '6px 0 10px', fontFamily: F.sans, lineHeight: 1.5 }}>
                Add any specific details about how you want your course. E.g. "Focus on exam board AQA", "Use analogies from sports", "Skip introductory chapters".
              </p>
              <textarea
                value={prefs.customInstructions || ''}
                onChange={e => setPrefs(p => ({ ...p, customInstructions: e.target.value }))}
                rows={4}
                placeholder="Tell the AI exactly how to personalise your course..."
                style={{
                  width: '100%', padding: '13px 16px', background: T.card2,
                  border: `1px solid ${T.border}`, borderRadius: 10, color: T.text,
                  fontSize: 14, fontFamily: F.sans, caretColor: T.violet,
              ...
              {prefs.customInstructions && (
                <div style={{ textAlign: 'right', marginTop: 4 }}>
                  <MonoLabel size={9} color={T.muted}>{prefs.customInstructions.length} / 500</MonoLabel>
                </div>
              )}
            </div>
